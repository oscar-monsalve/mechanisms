...
def is_grashof(a: float, b: float, c: float, d: float) -> bool:
    ...
    if min_len + max_len < other_links_sum:
        print("""Yes, it is a class I Grashof mechanism. Meaning at least one of the linkages will describe
        a full rotation.\n""")
        return True
    if min_len + max_len == other_links_sum:
        print("""Yes, it is a class III Grashof mechanism. Meaning at least one of the linkages will describe
        a full rotation and colineal positions will occur.\n""")
        return True
    if min_len + max_len > other_links_sum:
